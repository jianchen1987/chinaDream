...
-(void)deleteCharctor{
    ...
    if(_deleteBtnClickBlock != nil) {
        [_modelArr removeLastObject];
        _deleteBtnClickBlock(_contentStr,_modelArr);
...
-(void)responseEmotionKeyBoardClick:(EmotionModel *)emotionModel{
    ...
    NSLog(@"type = %ld",(long)emotionModel.type);
    NSLog(@"chs = %@",emotionModel.chs);
    NSLog(@"cht = %@",emotionModel.cht);
    NSLog(@"git = %@",emotionModel.git);
    NSLog(@"code = %@",emotionModel.code);
    NSLog(@"png = %@",emotionModel.png);
    NSLog(@"_________________");
    //拼接输入的字符
    if (1 == emotionModel.type) {
        [_contentStr appendString:emotionModel.code];
    }else {
        [_contentStr appendString:emotionModel.chs];
    }
    [_modelArr addObject:emotionModel];
    NSLog(@"contentStr = %@",_contentStr);
